...
if [ ! -z "$GERRIT_CHANGES" ]
then
    CHANGE_NUMBER=`echo $GERRIT_CHANGES|cut -d '/' -f 4`
    echo "Triggered by: https://$SITE/$CHANGE_NUMBER"
fi

if [ ! -z "$GERRIT_REFSPEC" ]
then
    CHANGE_NUMBER=`echo $GERRIT_REFSPEC|cut -d '/' -f 4`
    echo "Triggered by: https://$SITE/$CHANGE_NUMBER"
fi
...
function merge_change {
    ...
    until git fetch ssh://jenkinsbot@$SITE:29418/$PROJECT.git $REFSPEC
    do
        COUNT=$(($COUNT + 1))
        logger -p user.warning -t 'gerrit-git-prep' FAILED: git fetch git fetch ssh://jenkinsbot@$SITE:29418/$PROJECT.git $REFSPEC COUNT: $COUNT
...
if [[ ! -e .git ]]
then
    git clone ssh://jenkinsbot@$SITE:29418/$GERRIT_PROJECT.git .
fi
