...
def merge_change(site, gerrit_project, gerrit_refspec)
  ...
  while attempts < max_attempts
    begin
      run_command("git fetch ssh://jenkinsbot@#{site}:29418/#{gerrit_project}.git #{gerrit_refspec}")
      break
    rescue 
...
if !File.directory?(dot_git_dir)
  run_command("git clone ssh://jenkinsbot@#{site}:29418/#{ENV['GERRIT_PROJECT']}.git .")
end
begin
